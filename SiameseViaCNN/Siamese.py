...
class Siamese(object):
    def __init__(self):
        ...
        self.tf_inputA = tf.placeholder(tf.float32, [None, 112,92,1], name = 'inputA')
        self.tf_inputB = tf.placeholder(tf.float32, [None,112,92,1], name = 'inputB')

        # labels for the image pair # 1: similar, 0: dissimilar
        self.tf_Y = tf.placeholder(tf.float32, [None,], name = 'Y')
        self.tf_YOneHot = tf.placeholder(tf.float32, [None,40], name = 'YoneHot')
    ...
    def network(self, tf_input, trainable=True):#change for cnn
    ...
    def trainSiamese(self, x, y, epochs, batchSize=10): #change
        # Train the network
    ...
    def trainSiameseForClassification(self, x, y, epochs, batchSize=10): #change
        ...
        for i in range(epochs):
            ...
            for j in range(0,numSamples,2*batchSize):
                input1 = x[j:j+batchSize]
                y1=y[j:j+batchSize]
                y1c = to_categorical(y1,num_classes=40) # convert labels to one hot
